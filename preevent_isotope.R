...
e1 <- disprecipplot(event1, event1$dateTime, event1$X_00060_00000, e1p, e1p$dateTime, e1p$prcp.mm, .3,
  ...
  annotate("text",x = as.POSIXct("2022-05-02 00:00:00"), y = 45, size = 7, label = "E/I = -0.004")
e2 <- disprecipplot(event2, event2$dateTime, event2$X_00060_00000, e2p, e2p$dateTime, e2p$prcp.mm, .2,
  ...
  annotate("text",x = as.POSIXct("2022-06-05 00:00:00"), y = 190,size = 7,label = "E/I = 0.057")
e3 <- disprecipplot(event3, event3$dateTime, event3$X_00060_00000, e3p, e3p$dateTime, e3p$prcp.mm, .2,
  ...
  annotate("text",x = as.POSIXct("2022-06-16 07:00:00"), y = 190, size = 7,label = "E/I = 0.018")
e4 <- disprecipplot(event4, event4$dateTime, event4$X_00060_00000, e4p, e4p$dateTime, e4p$prcp.mm, .05,
  ...
  annotate("text",x = as.POSIXct("2022-07-04 12:00:00"), y = 40,size = 7,label = "E/I = 0.007")
e5 <- disprecipplot(event5, event5$dateTime, event5$X_00060_00000, e5p, e5p$dateTime, e5p$prcp.mm, .6,
  ...
  annotate("text",x = as.POSIXct("2022-07-12 18:00:00"), y = 13,size = 7, label = "E/I = 0.015")
e6 <- disprecipplot(event6, event6$dateTime, event6$X_00060_00000, e6p, e6p$dateTime, e6p$prcp.mm, 1.25,
  ...
  annotate("text",x = as.POSIXct("2022-08-17 06:00:00"), y = 13, size = 7, label = "E/I = -0.065")
e7 <- disprecipplot(event7, event7$dateTime, event7$X_00060_00000, e7p, e7p$dateTime, e7p$prcp.mm, .09,
  ...
  annotate("text",x = as.POSIXct("2023-06-26 12:00:00"), y = 65,size = 7, label = "E/I = -0.002")
e8 <- disprecipplot(event8, event8$dateTime, event8$X_00060_00000, e8p, e8p$dateTime, e8p$prcp.mm, 1.1,
  ...
  annotate("text",x = as.POSIXct("2023-08-08 23:00:00"), y = 17,size =7, label = "E/I = -0.049")
e9 <- disprecipplot(event9, event9$dateTime, event9$X_00060_00000, e9p, e9p$dateTime, e9p$prcp.mm, 1.1,
  ...
  annotate("text",x = as.POSIXct("2023-09-08 23:00:00"), y = 17,size = 7,label = "E/I = -0.025")
e10 <- disprecipplot(event10, event10$dateTime, event10$X_00060_00000, e10p, e10p$dateTime, e10p$prcp.mm, .05,
  ...
  annotate("text",x = as.POSIXct("2024-06-12 12:00:00"), y = 200,size =7,label = "E/I = 0.022")
e11 <- disprecipplot(event11, event11$dateTime, event11$X_00060_00000, e11p, e11p$dateTime, e11p$prcp.mm, .3,
  ...
  annotate("text",x = as.POSIXct("2024-07-07 12:00:00"), y = 23,size = 7,label = "E/I = 0.013")
e12 <- disprecipplot(event12, event12$dateTime, event12$X_00060_00000, e12p, e12p$dateTime, e12p$prcp.mm, 1.3,
  ...
  annotate("text",x = as.POSIXct("2024-08-04 12:00:00"), y = 13,size =7,label = "E/I = -0.009")
...
event7 <- usgsdis("13010065", "00060","2023-06-10", "2023-07-20") %>% 
  mutate(date = as.Date(dateTime),
         time = format(as.POSIXct(dateTime), format = "%H:%M:%S"),,
         time = as.POSIXct(time, format = "%H:%M:%S"),
         hour = as.numeric(format(time, format = "%H")),
         minute = as.numeric(format(time, format = "%M")),
         second = as.numeric(format(time, format = "%S"))) %>% 
  group_by(date, hour) %>% 
  summarise(X_00060_00000 = mean(X_00060_00000))%>%
  mutate(time = hms::as.hms(hour))
e7p <- precip(upper.snake.precip.month, '2023-06-10', '2023-07-20')
